<script>
  ...
  const styles = {
    ...
    info: "border-hairline bg-white text-muted  dark:bg-elevated text-fg"
  };
</script>
...
<div class="fixed bottom-4 right-4 z-50 flex w-96 max-w-[90vw] flex-col gap-2">
  {#each $toasts as t (t.id)}
    <button
      class="border px-4 py-2.5 text-left text-sm {styles[t.type] || styles.info}" style="border-radius: var(--radius); border-width: var(--border); box-shadow: var(--elevation)"
