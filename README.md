...
Flock is the account app of [ewe OS](https://github.com/prj786/ewe-os)
(RFC-006). Komble installs apps and records them in the one file;
ewe-settings edits the one file; **Flock moves the one file and your folders
...
| Machines | your flock — every machine that backed up to the account, with its ewe version and app count |
| Folders | which folders sync where (Flock 0.2, on `nextcloudcmd`) |

Tray: state icon (idle · syncing · conflict · offline · signed out), menu
with Sync now · Pause auto-sync · Open Flock · Quit. Left-click opens the
window.
...
- **The one file** `~/.config/ewe/ewe.conf` and, in your account,
  ...
  `ewe/machines/<name>.json` (the flock).
...
credentials, and a provider's signup form is theirs. Flock links to signup
pages; you create the account in the browser and sign in here.

## How it works

Flock is a thin UI over the ewe tools, run as argv, never through a shell:
...
`ewe-flock --hidden` starts tray-only (the autostart unit does this);
`ewe-flock --check` prints one JSON line with the tools it found, the
account status, the sync status and the flock — for support and the smoke
test.
...
`ewe-flock` is available — runs `ewe-flock --check` against that state.
